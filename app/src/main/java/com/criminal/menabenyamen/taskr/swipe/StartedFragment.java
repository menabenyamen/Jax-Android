...
public class StartedFragment extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        recyclerView = (RecyclerView) getView().findViewById(R.id.recycler_view_started);
        ...
        if(ConnectedToInternet.isOnline(getContext())){
            ...
            HashSet<WorkItem> hashSet = new HashSet<>(workItemRepository.getWorkItemsWhihStarted());

            for (WorkItem workItem : hashSet){
                if (workItem.getStatus().equals("STARTED") &&
            ...
            adapter = new WorkItemListAdapter(sqlRepository.getWorkItemsWhihStarted(), getContext(), new WorkItemListAdapter.onCLickResultListener() {
                @Override
                public void onClickResult(WorkItem workitem) {
                    callBacks.onListItemClicked(workitem);
                }
